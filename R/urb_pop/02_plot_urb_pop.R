...
#### plot1 - populacao por situacao de municipio

pop_br <- readr::read_rds("../../data/urbanformbr/population/table_202_ibge.rds")[,tipo_valor := "total"][,valor := valor/10^6]
...
ggsave("./figures/urb_pop/faixa_br_box.png",scale=1,
...
most_pop <- pop_muni[ano == 2010][order(valor,decreasing = TRUE)]
most_pop <- most_pop[1:12,municipio_codigo]
...
pop_muni_df[,municipio := str_sub(string = municipio,start = 1,end = nchar(municipio)-5)]
pop_muni_df[,status_metro := fifelse(municipio_codigo %in% most_pop,
                                     "Principal","Periferia metropolitana")]
# add muni_principal
pop_muni_df[pop_muni_df[status_metro == "Principal"],
            on = "name_metro",cidade_principal := i.municipio]
...
# Spread data from long to wide
pop_muni_df3 <- data.table::dcast(data = pop_muni_df2,
                                  formula = code_amc +
                                    cidade_principal +
                                    status_metro ~ ano,
                                  value.var = "valor",
                                  fun = sum)
pop_muni_df3
names(pop_muni_df3)[4:9] <- paste0('valor_', names(pop_muni_df3)[4:9])
head(pop_muni_df3)

# sum valores
pop_muni_df3 <- pop_muni_df3[,.(
  valor_1970 = sum(valor_1970,na.rm = TRUE),
  valor_1980 = sum(valor_1980,na.rm = TRUE),
  valor_1991 = sum(valor_1991,na.rm = TRUE),
  valor_2000 = sum(valor_2000,na.rm = TRUE),
  valor_2010 = sum(valor_2010,na.rm = TRUE),
  valor_2020 = sum(valor_2020,na.rm = TRUE)
), by = .(cidade_principal,status_metro)]
# weight

pop_muni_df3[,r70_80 := 100 * ((valor_1980 / valor_1970)^(1/(1980-1970)) - 1),
             by = .(cidade_principal,status_metro)]
pop_muni_df3[,r80_91 := 100 * ((valor_1991 / valor_1980)^(1/(1991-1980)) - 1),
             by = .(cidade_principal,status_metro)]
pop_muni_df3[,r91_00 := 100 * ((valor_2000 / valor_1991)^(1/(2000-1991)) - 1),
             by = .(cidade_principal,status_metro)]
pop_muni_df3[,r00_10 := 100 * ((valor_2010 / valor_2000)^(1/(2010-2000)) - 1),
             by = .(cidade_principal,status_metro)]
pop_muni_df3[,r10_20 := 100 * ((valor_2020 / valor_2010)^(1/(2020-2010)) - 1),
             by = .(cidade_principal,status_metro)]
pop_muni_df3 <- data.table::copy(pop_muni_df3)[,.SD[1],
                                               by = .(cidade_principal,
                                                      status_metro)]

pop_muni_df3[1:3]
tmp_plot <- data.table::copy(pop_muni_df3)

tmp_plot <- data.table::melt(data = tmp_plot,
                             id_vars = c('cidade_principal','status_metro'),
                             # measure.vars = list( 'taxa' = c('r70_80','r80_91','r91_00',
                             #                                 'r00_10','r10_20')),
                             measure.vars = list( 'taxa' = c('r70_80',
                                                             'r80_91',
                                                             'r91_00',
                                                             'r00_10',
                                                             'r10_20')),
...
tmp_plot$cidade_principal_f <- factor(tmp_plot$cidade_principal,
                                      levels = c("Brasília",
                                                 "Belém",
                                                 "Belo Horizonte",
                                                 "Curitiba",
...
ggplot(data = tmp_plot) +
  ...
  geom_point(aes(x = posicao_taxa_f,y = taxa,
                 color = status_metro_f),
             #position =  position_dodge(width = 0.5),
             size = 1.5) +
  geom_line(aes(x = posicao_taxa_f,
                y = taxa,
                color = status_metro_f,
                group = status_metro_f),
            #position =  position_dodge(width = 0.5),
            size=0.75) +
  geom_hline(yintercept = 0, color= "grey55", size =0,
             linetype= "dotted") +
  # facet

  facet_wrap(vars(cidade_principal_f),nrow = 4)+
  ...
  labs(x = "Intervalo de análise",
       ...
       <br>Faixas populacionais definidas conforme Áreas Mínimas Comparáveis (AMC), desde 1970.'
  ) +
  ...
  theme(legend.position = "right",
        ...
        axis.line.x = element_line(size = 0.5, color = "grey"),panel.grid.major = element_blank(),
