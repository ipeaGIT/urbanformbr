...
onlynumbersbase <- pca_regression_df_ready_to_use %>%
  dplyr::mutate(
    i_name_urban_concentration = NULL,
    i_name_uca_case = NULL,
    i_code_urban_concentration = NULL)

DataExplorer::plot_qq(onlynumbersbase)


DataExplorer::plot_qq(onlynumbersbase,nrow = 2, ncol = 2) +
  theme(axis.text =)
...
# to as.numeric
dt_colnames <- names(onlynumbersbase)
onlynumbersbase[,(dt_colnames) := lapply(.SD,as.numeric)]

## Data Transforming -----
## YEO JOHNSON TRANSFORMATIOn
basenumberyeo <- VGAM::yeo.johnson(y = onlynumbersbase
                                   , lambda = 0) ##ILHEUS DA PROBLEMA

basenumberyeo <- basenumberyeo  %>%
  dplyr::mutate(D_SistBMT = onlynumbersbase$D_SistBMT,
                d_isolated_muni = onlynumbersbase$d_isolated_muni,
                d_large_uca_pop = onlynumbersbase$d_large_uca_pop)

# remove NA values
basenumberyeo <- basenumberyeo[complete.cases(basenumberyeo)]

## HIPERBOLIC TRANSFORMATION
basenumberhip <- onlynumbersbase %>%
  dplyr::mutate_if(is.numeric,~log(((.x + .x^2+1)^(1/2))))

basenumberhip  <- basenumberhip  %>%
  dplyr::mutate(D_SistBMT = onlynumbersbase$D_SistBMT,
                d_isolated_muni = onlynumbersbase$d_isolated_muni,
                d_large_uca_pop = onlynumbersbase$d_large_uca_pop)
...
prblmtc_detect <- function(x) {
  ...
  return(ifelse(min(x) <= 0,FALSE,TRUE))
...
matplot(basenumberhip$y_fuel_consumption_per_capita_2010
        , onlynumbersbase$y_fuel_consumption_per_capita_2010
        , type = "l", ylim = c(-10, 500)
        , lwd = 2, lty = 1:lltry
...
rf_safuellog1000 <- readr::read_rds("../urbanformbr/Outputs/Caret/rf_safuellog1000iteracoes ")

rf_sacomutelog1000 <- readr::read_rds("../urbanformbr/Outputs/Caret/rf_sacomutelog1000iteracoes")

###### CARET SAFS (FEATURE ANNEALING) CALIBRATION

xlog <- basenumberlog %>%
  dplyr::mutate(y_fuel_consumption_per_capita_2010 = NULL,
...
comboInfo <- caret::findLinearCombos(xlog)
comboInfo

xhip <- onlynmbrhip %>%
  dplyr::mutate(x_prop_car_pes = NULL,
                x_prop_motorcycle_pes = NULL,
                x_prop_car_and_motorcycle_pes = NULL)
...
rfsacontrl <- caret::safsControl(functions = rfSA,
                                 method = "repeatedcv",
                                 repeats = 10,
                                 improve = 15)
...
rf_safuellog1000 <- caret::safs(x = xlog,
                                y = y1log,
                                iters = 1000,
...
set.seed(0)
rf_sacomutelog1000 <- caret::safs(x = xlog,
                                  y = y2log,
                                  iters = 1000,
                                  safsControl = rfsacontrl)

readr::write_rds(x = rf_sacomutelog1000
                 , file = "rf_sacomutelog1000iteracoes.rds")
...
dfxviffuellog <- dplyr::filter(dfxviffuellog, vif(regfuellog3) <= 10)
...
dfxvifcomutelog <- dplyr::filter(dfxvifcomutelog, vif(regcomutelog) <= 10)
...
cor.test(pca_regression_df_ready_to_use$y_wghtd_mean_commute_time
         , pca_regression_df_ready_to_use$x_wghtd_mean_household_income_per_capita)

dfx <- select(opsetcomutehip)

ggcorrplot::ggcorrplot(cor(dfx),tl.cex = 8)
...
stargazer::stargazer(regfuellog200
                     , regfuellog500
                     , regfuellog1000
                     , type = 'html'
                     , out = "caretfuelregs")

stargazer::stargazer(regfuellogmtclseed10
                     , regfuellogmtclseed100
                     , regfuellogmtcl3
                     , type = 'html'
                     , out = "cleaneregs")

car::scatterplot(y_wghtd_mean_commute_time ~
                   log(x_urban_extent_size_2014) | D_SistBMT
                 , data = pca_regression_df_ready_to_use
                 , xlab = 'Extensão Urbana em KM'
                 , ylab = 'Tempo Médio de Comutação ao Trabalho')
