...
# this script downloads, unzips and saves data from faces de logradouros / IBGE datasets
...
download_logradouros <- function(state_abbrev) {
  ...
  download_dir = sprintf("../../data-raw/logradouros/%s", state_abbrev)
...
download_logradouros_2019 <- function(state_abbrev) {
  ...
  # make sure local directory exists
  download_dir = sprintf("../../data-raw/logradouros_2019/%s", state_abbrev)
  if (!dir.exists(download_dir)) {
    dir.create(download_dir, recursive = TRUE)
  }
...
walk(states_sf$abbrev_state, download_logradouros)
