...
regression_df %>%
  ggplot(aes(x=x_pop_2010, y=y_wghtd_mean_commute_time)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10()

regression_df %>%
  select(x_wghtd_mean_household_income_per_capita, x_urban_extent_size_2014, x_pop_2010,
         y_wghtd_mean_commute_time, y_energy_per_capita) %>%
  cor() %>%
  View()

regression_df <- regression_df %>%
  mutate(pop_log = log(x_pop_2010),
         urbanextent_log = log(x_urban_extent_size_2014),
         commute_log = log(y_wghtd_mean_commute_time),
         fuel_log = log(y_energy_per_capita),
         income_log = log(x_wghtd_mean_household_income_per_capita))

lm(y_energy_per_capita ~ x_wghtd_mean_household_income_per_capita + x_prop_autos_dom, regression_df) %>% summary()
lm(fuel_log ~ income_log + x_prop_autos_dom, regression_df) %>% summary()


lm(y_wghtd_mean_commute_time ~ x_pop_2010 * urbanextent_log, regression_df) %>% summary()
...
metrics_df %>%
  filter(year == "2014") %>%
  ggplot(aes(y=density01km, x = compacity, color = proportion_largest_patch, size = population)) +
...
plot_vars_group(metrics_df, "compacity", "density01km")
